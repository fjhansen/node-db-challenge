...
1. Relational Database: a general definition for a column based database that has data that can be linked together within the database.
2. SQL: the query based language that is used to commuinicate with relational databases

- [x] Why do tables need a `primary key`?

a primary key is needed to uniqely identify a table within a database. it is also needed to join one table to another.
...
a table that acts as a bridge between the two tables so it can accurately store the relationship between the two tables.
...
## Notes
seeding really helped me think about structuring everything! didnt have time to do the full stretch but i chose to do seeding beforehand for a good workflow.
